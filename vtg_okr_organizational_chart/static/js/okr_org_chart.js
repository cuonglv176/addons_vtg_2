...
odoo.define("vtg_okr_organizational_chart.okr_org_chart", function (require) {
  "use strict";
  ...
  var OrgChartDepartment = AbstractAction.extend({
    ...
    init: function(parent, context){
      this._super(parent, context);
        var self = this;
        if (context.tag == 'okr_organization_chart') {
            this._rpc({
            route: '/okr/get/employees',
        }).then(function (result) {
            self._rpc({
                model: 'okr.organizational.chart',
    ...
    render: function() {
        ...
        var self = this;
        var org_chart = QWeb.render('vtg_okr_organizational_chart.org_chart_template', {
            widget: self,
        });
  ...
  core.action_registry.add('okr_organization_chart', OrgChartDepartment);
  window.reload()
