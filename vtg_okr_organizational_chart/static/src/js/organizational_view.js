odoo.define('vtg_okr_organizational_chart.view_chart', function (require){
"use strict";
...
var OKROrganizationalChart =  AbstractAction.extend({

    contentTemplate: 'OrganizationalOKRChart',
    events: {
    ...
    init: function(parent, context) {
        ...
        this.renderOKRDetails();
    },
    renderOKRDetails: function (){
        var okr_id = 1
        var self = this;
        console.log("hhhhh",this)
        this._rpc({
            route: '/okr/get/parent/employee',
        }).then(function (result) {
            self.parent_len = result[1];
            $.ajax({
                url: '/okr/get/parent/child',
                type: 'POST',
    ...
    _getChild_data: function(events){
        ...
        if(events.target.parentElement.className){
            ...
            if (this.check_child[0]){
                ...
                if (child_length == 1){
                    this._rpc({
                        route: '/okr/get/parent/colspan',
                        params: {
                            okr_id: parseInt(this.id),
                        },
    ...
    core.action_registry.add('okr_organization_dashboard', OKROrganizationalChart);
