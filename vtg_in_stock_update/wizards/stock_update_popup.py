...
class StockUpdatePopup(models.TransientModel):
    ...
    def action_update_in_stock(self):
        ...
        while row < sheet.nrows:
            ...
            main_commit = sheet.cell(row, 6).value
            sub_commit = sheet.cell(row, 7).value
            product_id = self.env['product.product'].search([('default_code', '=', id)])
            ...
            product_id.update({
                ...
                'ampunt_for_workers': main_commit,
                'ampunt_for_sub_workers': sub_commit,
            })
            row += 1
