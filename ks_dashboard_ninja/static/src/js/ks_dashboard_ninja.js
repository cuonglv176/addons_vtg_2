...
//              });
        ...
        ksOnLayoutSelection: function (layout_id) {
            ...
            _(this.gridstackConfig).each((x, y) => {
                self.grid.update(self.$el.find(".grid-stack-item[gs-id=" + y + "]")[0], {
                    x: x['x'],
                    y: x['y'],
                    w: x['w'],
                    h: x['h'],
                ...
                self.call('bus_service', 'onNotification', self, function (notifications) {
                    _.each(notifications, (function (notification) {
                        if (notification.hasOwnProperty('type') && notification['type'].type === "ks_dashboard_ninja.notification" && self.ks_mode === 'active') {
                            var item_to_update = _(notification.type.changes).filter((x) => {
                                return self.ks_dashboard_data.ks_dashboard_items_ids.indexOf(x) >= 0
                            });
                            if (item_to_update) {
                                ...
                                var update_notification_ids = _(item_to_update).filter((x) => {
                                    return self.ks_dashboard_data.ks_item_data[x].ks_auto_update_type === 'ks_live_update' && self.ks_dashboard_data.ks_item_data[x].ks_show_live_pop_up === true
                                });
...
//                result = self.normalize_dn_data(result);
        ...
        ksRenderDashboard: function () {
            ...
            var $ks_header = $(QWeb.render('ksDashboardNinjaHeader', {
                ...
                ks_dn_pre_defined_filters: _(self.ks_dashboard_data.ks_dashboard_pre_domain_filter).values().sort(function (a, b) {
        ...
        ksChartColors: function (palette, ksMyChart, ksChartType, ksChartFamily, stack, semi_circle, ks_show_data_value, chart_data, item) {
            ...
            switch (palette) {
                ...
                case 'default':
                    ...
                    var color_set = ['#006F67', '#F68A1F', '#fdc233', '#53cfce', '#36a2ec', '#8a79fd', '#b1b5be', '#1c425c', '#8c2620', '#71ecef', '#0b4295', '#f2e6ce', '#1379e7']
...
//                            framework.unblockUI();
        ...
        _ksSaveCurrentLayout: function () {
            var self = this;
            var grid_config = {}
            if (self.ks_dashboard_data.ks_dashboard_items_ids.length > 0) {
                var grid_config = self.ks_get_current_gridstack_config();
            }
                ...
                switch (field.ks_data_comparison) {
                    case "None":
                        if (field.ks_multiplier_active) {
                            ...
                            var count_tooltip = String(count_total * field.ks_multiplier);
                            ...
                            var count = String(KsGlobalFunction.ksNumFormatter(count_total * field.ks_multiplier, 1));
                            ...
                            item_info['count'] = String(KsGlobalFunction._onKsGlobalFormatter(count_total * field.ks_multiplier, field.ks_data_formatting, field.ks_precision_digits));
                            ...
                            var count_tooltip = String(count_total);
                            var count = String(KsGlobalFunction.ksNumFormatter(count_total, 1));
                            item_info['count'] = String(KsGlobalFunction._onKsGlobalFormatter(count_total, field.ks_data_formatting, field.ks_precision_digits));
                        }
                    ...
                    case "Ratio":
                        ...
                        if (item.ks_data_formatting == 'exact') {
                            ...
                            if (count_1 && count_2) {
                                ...
                                item_info['count'] = String(field_utils.format.float(count_1 * field.ks_multiplier, Float64Array, {digits: [0, 0]})) + "/" + String(field_utils.format.float(count_2 * field.ks_multiplier, Float64Array, {digits: [0, 0]}));
